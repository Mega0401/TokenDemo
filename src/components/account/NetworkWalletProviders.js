...
const networks = [
  ...
  { label: "BSC Testnet", value: "bsc", icon: <BinanceLogo width={60} /> },
];
...
const NetworkWalletProviders = ({
  ...
  const { loginMetamask, loginInjected } = useWalletConnector();
  ...
  const connectWallet = async (walletprovider) => {
    ...
    // Normalize legacy Rabby choice to injected (Rabby option was removed)
    const normalized =
      walletprovider === "rabby_eth" ? "injected_eth" : walletprovider === "rabby_bsc" ? "injected_bsc" : walletprovider;
    if (typeof window !== "undefined" && !window.ethereum) {
      console.error("No wallet detected. Install MetaMask.");
      return;
    }
    localStorage.setItem("connected", "true");

    const loginForInjected = () => {
      if (loginInjected) loginInjected();
      else if (loginMetamask) loginMetamask();
    };
    switch (normalized) {
      case "injected_eth":
        setWalletProvider(normalized);
        setNet(0);
        loginForInjected();
        break;
      case "injected_bsc":
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("connected") === "true" && saved) {
      connectWallet(saved);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
